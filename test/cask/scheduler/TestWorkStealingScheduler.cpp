...
class WorkStealingSchedulerTest : public ::testing::TestWithParam<int> {
protected:

    void SetUp() override {
        sched = std::make_shared<WorkStealingScheduler>(GetParam());
    }
    ...
    std::shared_ptr<WorkStealingScheduler> sched;
};

TEST_P(WorkStealingSchedulerTest, IdlesAtStart) {
    EXPECT_TRUE(sched->isIdle());
}

TEST_P(WorkStealingSchedulerTest, SubmitSingle) {
    std::mutex mutex;
    mutex.lock();
...
TEST_P(WorkStealingSchedulerTest, SubmitBulk) {
...
TEST_P(WorkStealingSchedulerTest, SubmitAfter) {
    std::mutex mutex;
    mutex.lock();
...
TEST_P(WorkStealingSchedulerTest, SubmitAfterCancel) {
    std::mutex mutex;
    mutex.lock();
...
TEST_P(WorkStealingSchedulerTest, RegistersCallbackAfterCancelled) {
    std::mutex mutex;
    mutex.lock();
...
TEST_P(WorkStealingSchedulerTest, RunsShutdownCallbackAfterTimerTaskCompletion) {
    bool shutdown = false;
...
TEST_P(WorkStealingSchedulerTest, RunsShutdownImmediatelyCallbackIfTimerAlreadyFired) {
    bool shutdown = false;
    std::mutex mutex;
    mutex.lock();
...
INSTANTIATE_TEST_SUITE_P(Scheduler, WorkStealingSchedulerTest, ::testing::Values(1, 2, 4, 16));
