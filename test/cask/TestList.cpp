...
TEST(List, Prepend) {
    auto list = List<int>::empty()->prepend(1)->prepend(2)->prepend(3);
...
TEST(List, Append) {
    auto list = List<int>::empty()->append(1)->append(2)->append(3);
...
TEST(List, DropWhilePartialMatch) {
    auto list = List<int>::empty()->append(1)->append(2)->append(3)->dropWhile([](auto value) {
        return value <= 2;
    });
