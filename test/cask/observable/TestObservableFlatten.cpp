...
TEST(ObservableFlatten, FlattensValue) {
    ...
    auto result = flattened->last().run(Scheduler::global())->await();
...
TEST(ObservableFlatten, FlattensError) {
    ObservableRef<ObservableRef<int, float>, float> nested =
        Observable<ObservableRef<int, float>, float>::pure(Observable<int, float>::raiseError(1.23));
    ObservableRef<int, float> flattened = nested->template flatten<int>();

    auto result = flattened->last().failed().run(Scheduler::global())->await();
