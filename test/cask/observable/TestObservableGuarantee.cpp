...
TEST(ObservableGuarantee, RunsOnCompletion) {
    ...
    auto result = Observable<int, float>::pure(123)->guarantee(task)->last().run(Scheduler::global())->await();
...
TEST(ObservableGuarantee, RunsOnUpstreamComplete) {
    ...
    auto result = Observable<int, float>::eval([] {
                      return 123;
                  })
                      ->guarantee(task)
                      ->last()
...
TEST(ObservableGuarantee, RunsOnError) {
    ...
    auto result =
        Observable<int, float>::raiseError(1.23)->guarantee(task)->last().failed().run(Scheduler::global())->await();
...
TEST(ObservableGuarantee, ErrorOnce) {
    ...
    REQUIRE_CALL(*mockDownstream, onError(1.23f)).RETURN(Task<None, None>::none());
...
TEST(ObservableGuarantee, CancelOnce) {
    ...
    REQUIRE_CALL(*mockDownstream, onCancel()).RETURN(Task<None, None>::none());
