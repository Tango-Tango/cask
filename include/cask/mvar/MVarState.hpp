...
template <class T, class E>
MVarState<T, E>::MVarState(std::shared_ptr<Scheduler>&& sched,
                           std::optional<T>&& valueOpt,
                           ListRef<PendingPut>&& pendingPuts,
                           ListRef<PromiseRef<T, E>>&& pendingTakes)
    : sched(sched)
...
std::tuple<MVarState<T, E>, bool, std::function<void()>> MVarState<T, E>::tryPut(const T& value) const {
    ...
    if (!filteredTakes->is_empty()) {
        ...
        return std::make_tuple(
            MVarState(sched, valueOpt, pendingPuts, filteredTakes->tail()), true, [takePromise, value] {
                takePromise->success(value);
            });
    } else if (!valueOpt.has_value()) {
        return std::make_tuple(MVarState(sched, value, pendingPuts, filteredTakes), true, [] {});
    } else {
        return std::make_tuple(MVarState(sched, valueOpt, pendingPuts, filteredTakes), false, [] {});
    }
}
...
std::tuple<MVarState<T, E>, Task<None, E>> MVarState<T, E>::put(const T& value) const {
    ...
    if (!completed) {
        ...
        return std::make_tuple(
            MVarState(
                nextState.sched, nextState.valueOpt, nextState.pendingPuts->append(pending), nextState.pendingTakes),
...
std::tuple<MVarState<T, E>, Task<T, E>> MVarState<T, E>::take() const {
    ...
    if (valueOpt.has_value()) {
        return std::make_tuple(MVarState(sched, {}, filteredPuts, pendingTakes), Task<T, E>::pure(*valueOpt));
    } else if (!filteredPuts->is_empty()) {
        auto pending = *(filteredPuts->head());
        auto putPromise = std::get<0>(pending);
        auto value = std::get<1>(pending);
        return std::make_tuple(MVarState(sched, valueOpt, filteredPuts->tail(), pendingTakes),
        ...
        return std::make_tuple(MVarState(sched, valueOpt, filteredPuts, pendingTakes->append(promise)),
