 ...
 */
template <class TI, class TO, class E> class FlatMapObservable final : public Observable<TO, E> {
public:
    FlatMapObservable(const std::shared_ptr<const Observable<TI, E>>& upstream,
...
FlatMapObservable<TI, TO, E>::FlatMapObservable(const std::shared_ptr<const Observable<TI, E>>& upstream,
                                                const std::function<ObservableRef<TO, E>(const TI&)>& predicate)
    : upstream(upstream)
    , predicate(predicate) {}

template <class TI, class TO, class E>
FiberRef<None, None> FlatMapObservable<TI, TO, E>::subscribe(const std::shared_ptr<Scheduler>& sched,
