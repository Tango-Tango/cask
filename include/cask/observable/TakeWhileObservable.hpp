 ...
 */
template <class T, class E> class TakeWhileObservable final : public Observable<T, E> {
public:
    TakeWhileObservable(const std::shared_ptr<const Observable<T, E>>& upstream,
...
TakeWhileObservable<T, E>::TakeWhileObservable(const std::shared_ptr<const Observable<T, E>>& upstream,
                                               const std::function<bool(const T&)>& predicate,
                                               bool inclusive)
    : upstream(upstream)
    , predicate(predicate)
...
template <class T, class E>
FiberRef<None, None> TakeWhileObservable<T, E>::subscribe(const std::shared_ptr<Scheduler>& sched,
                                                          const std::shared_ptr<Observer<T, E>>& observer) const {
