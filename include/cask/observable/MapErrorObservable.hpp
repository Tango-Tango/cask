...
public:
    ...
    FiberRef<None, None> subscribe(const std::shared_ptr<Scheduler>& sched,
                                   const std::shared_ptr<Observer<T, EO>>& observer) const override;
...
template <class T, class EI, class EO>
MapErrorObservable<T, EI, EO>::MapErrorObservable(const std::shared_ptr<const Observable<T, EI>>& upstream,
                                                  const std::function<EO(const EI&)>& predicate)
    : upstream(upstream)
    , predicate(predicate) {}

template <class T, class EI, class EO>
FiberRef<None, None> MapErrorObservable<T, EI, EO>::subscribe(const std::shared_ptr<Scheduler>& sched,
                                                              const std::shared_ptr<Observer<T, EO>>& observer) const {
