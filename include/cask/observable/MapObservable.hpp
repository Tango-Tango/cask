...
public:
    MapObservable(const ObservableConstRef<TI,E>& upstream, std::function<TO(TI&&)>&& predicate);
...
template <class TI, class TO, class E>
MapObservable<TI,TO,E>::MapObservable(const ObservableConstRef<TI,E>& upstream, std::function<TO(TI&&)>&& predicate)
    : upstream(upstream)
