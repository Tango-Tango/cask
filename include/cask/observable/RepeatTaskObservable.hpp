...
template <class T, class E>
FiberRef<None, None> RepeatTaskObservable<T, E>::subscribe(const std::shared_ptr<Scheduler>& sched,
                                                           const std::shared_ptr<Observer<T, E>>& observer) const {
    std::function<Task<Ack, None>(T)> pushToObserver = [observer = observer](T value) -> Task<Ack, None> {
        return observer->onNext(value);
    };

    std::function<Task<Ack, None>(E)> pushError = [observer = observer](E error) -> Task<Ack, None> {
        return observer->onError(error).template map<Ack>([](auto) {
            return Stop;
        });
    };

    return task.template flatMapBoth<Ack, None>(pushToObserver, pushError)
        .restartUntil([](auto ack) {
            return ack == Stop;
        })
