...
template <class T, class E>
FilterObservable<T, E>::FilterObservable(const std::shared_ptr<const Observable<T, E>>& upstream,
                                         const std::function<bool(const T&)>& predicate)
    : upstream(upstream)
    , predicate(predicate) {}

template <class T, class E>
FiberRef<None, None> FilterObservable<T, E>::subscribe(const std::shared_ptr<Scheduler>& sched,
