...
public:
    ...
    QueueState(std::shared_ptr<Scheduler>&& sched, uint32_t max_size, ListRef<T>&& values, ListRef<PendingPut>&& pendingPuts, ListRef<PromiseRef<T,E>>&& pendingTakes);
};
...
template <class T, class E>
QueueState<T,E>::QueueState(const std::shared_ptr<Scheduler>& sched, uint32_t max_size, const ListRef<T>& values, const ListRef<PendingPut>& pendingPuts, const ListRef<PromiseRef<T,E>>& pendingTakes)
    : sched(sched)
    , values(values)
...
template <class T, class E>
QueueState<T,E>::QueueState(std::shared_ptr<Scheduler>&& sched, uint32_t max_size, ListRef<T>&& values, ListRef<PendingPut>&& pendingPuts, ListRef<PromiseRef<T,E>>&& pendingTakes)
    : sched(sched)
    , values(values)
    , pendingPuts(pendingPuts)
    , pendingTakes(pendingTakes)
...
std::tuple<QueueState<T,E>,bool,std::function<void()>> QueueState<T,E>::tryPut(const T& value) const {
    ...
    if(!filteredTakes->is_empty()) {
        ...
        return std::make_tuple(
            QueueState(sched, max_size, values->append(value), pendingPuts, filteredTakes),
            true,
            []{}
        );
    } else {
...
std::tuple<QueueState<T,E>,Task<None,E>> QueueState<T,E>::put(const T& value) const {
    ...
    if(!completed) {
        ...
        return std::make_tuple(
            QueueState(nextState.sched, nextState.max_size, nextState.values, nextState.pendingPuts->append(pending), nextState.pendingTakes),
