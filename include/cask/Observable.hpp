...
public:
     ...
     */
    virtual FiberRef<None, None> subscribe(const std::shared_ptr<Scheduler>& sched,
                                           const std::shared_ptr<Observer<T, E>>& observer) const = 0;
    ...
    std::shared_ptr<Observable<T2, E2>> mapBothTask(const std::function<Task<T2, E2>(const T&)>& successPredicate,
                                                    const std::function<Task<T2, E2>(const E&)>& errorPredicate) const;
     ...
     */
    template <class T2, typename std::enable_if<std::is_assignable<ObservableRef<T2, E>, T>::value>::type* = nullptr>
...
template <class T, class E>
FiberRef<None, None> Observable<T, E>::subscribeHandlers(const std::shared_ptr<Scheduler>& sched,
                                                         const std::function<Task<Ack, None>(const T&)>& onNext,
                                                         const std::function<Task<None, None>(const E&)>& onError,
                                                         const std::function<Task<None, None>()>& onComplete,
                                                         const std::function<Task<None, None>()>& onCancel) const {
    auto observer = std::make_shared<observable::CallbackObserver<T, E>>(onNext, onError, onComplete, onCancel);
...
template <class T, class E>
template <class T2, class E2>
std::shared_ptr<Observable<T2, E2>>
Observable<T, E>::mapBothTask(const std::function<Task<T2, E2>(const T&)>& successPredicate,
                              const std::function<Task<T2, E2>(const E&)>& errorPredicate) const {
    auto self = this->shared_from_this();
...
template <class T, class E>
template <class T2>
std::shared_ptr<Observable<T2, E>>
Observable<T, E>::mapTask(const std::function<Task<T2, E>(const T&)>& predicate) const {
    auto self = this->shared_from_this();
...
template <class T, class E>
template <class T2, typename std::enable_if<std::is_assignable<ObservableRef<T2, E>, T>::value>::type*>
ObservableRef<T2, E> Observable<T, E>::flatten() const {
    return this->template flatMap<T2>([](auto inner) {
        return inner;
    });
}
