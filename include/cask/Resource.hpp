...
constexpr Resource<T, E> Resource<T, E>::make(const Task<T, E>& acquire, ReleaseFunction release) noexcept {
    return Resource<T, E>(acquire.template map<AllocatedResource>([release](T thing) {
...
constexpr Resource<T, E2> Resource<T, E>::mapError(std::function<E2(E)> predicate) const noexcept {
    return Resource<T, E2>(allocated
                               .template map<typename Resource<T, E2>::AllocatedResource>([predicate](auto result) {
                                   const auto& value = std::get<0>(result);
                                   const auto& release = std::get<1>(result);
                                   return std::make_tuple(value, release.template mapError<E2>(predicate));
                               })
...
constexpr Resource<T2, E> Resource<T, E>::flatMap(std::function<Resource<T2, E>(T)> predicate) const noexcept {
    ...
    std::function<TargetResourceTask(SourceResource)> composeResources = [predicate](SourceResource result) {
        T outerValue = std::get<0>(result);
        ReleaseTask outerRelease = std::get<1>(result);
        Resource<T2, E> innerResource = predicate(outerValue);

        std::function<TargetResource(TargetResource)> innerCompose = [outerRelease](auto innerAllocated) {
            const auto& innerValue = std::get<0>(innerAllocated);
            const auto& innerRelease = std::get<1>(innerAllocated);
            Task<None, E> fullRelease = innerRelease.template flatMap<None>([outerRelease](auto) {
                return outerRelease;
            });
            return std::make_tuple(innerValue, fullRelease);
        };

        TargetResourceTask composedAllocation = innerResource.allocated.template map<TargetResource>(innerCompose);
