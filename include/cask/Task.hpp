...
public:
     ...
     * @return A task wrapping the given deferal function.
     */
    constexpr static Task<T, E>
    deferAction(const std::function<DeferredRef<T, E>(const std::shared_ptr<Scheduler>&)>& predicate) noexcept;
     ...
     */
    constexpr static Task<T, E>
    deferFiber(const std::function<FiberRef<T, E>(const std::shared_ptr<Scheduler>&)>& predicate) noexcept;
    ...
    constexpr Task<T2, E2> flatMapBoth(const std::function<Task<T2, E2>(const T&)>& successPredicate,
                                       const std::function<Task<T2, E2>(const E&)>& errorPredicate) const noexcept;
     ...
     */
    template <class T2, typename std::enable_if<std::is_assignable<Either<T2, E>, T>::value>::type* = nullptr>
...
constexpr Task<T, E> Task<T, E>::defer(const std::function<Task<T, E>()>& predicate) noexcept {
    return Task<T, E>(fiber::FiberOp::thunk(predicate)->flatMap([](auto fiber_value) constexpr {
...
template <class T, class E>
constexpr Task<T, E>
Task<T, E>::deferAction(const std::function<DeferredRef<T, E>(const std::shared_ptr<Scheduler>&)>& predicate) noexcept {
    return Task<T, E>(fiber::FiberOp::async([predicate](auto sched) {
...
template <class T, class E>
constexpr Task<T, E>
Task<T, E>::deferFiber(const std::function<FiberRef<T, E>(const std::shared_ptr<Scheduler>&)>& predicate) noexcept {
    return Task<T, E>(fiber::FiberOp::async([predicate](auto sched) {
...
constexpr Task<T2, E> Task<T, E>::map(const std::function<T2(const T&)>& predicate) const noexcept {
    return Task<T2, E>(op->flatMap([predicate](auto fiber_value) {
        try {
            if (fiber_value.isValue()) {
...
constexpr Task<T, E2> Task<T, E>::mapError(const std::function<E2(const E&)>& predicate) const noexcept {
    return Task<T, E2>(op->flatMap([predicate](auto fiber_value) {
        try {
            if (fiber_value.isValue()) {
...
constexpr Task<T2, E> Task<T, E>::flatMap(const std::function<Task<T2, E>(const T&)>& predicate) const noexcept {
    return Task<T2, E>(op->flatMap([predicate](auto fiber_input) {
        try {
...
constexpr Task<T, E2> Task<T, E>::flatMapError(const std::function<Task<T, E2>(const E&)>& predicate) const noexcept {
    return Task<T, E2>(op->flatMap([predicate](auto fiber_input) {
        if (fiber_input.isValue()) {
...
template <class T, class E>
template <class T2, class E2>
constexpr Task<T2, E2>
Task<T, E>::flatMapBoth(const std::function<Task<T2, E2>(const T&)>& successPredicate,
                        const std::function<Task<T2, E2>(const E&)>& errorPredicate) const noexcept {
    if constexpr (std::is_same<E, E2>::value) {
        return Task<T2, E2>(op->flatMap([successPredicate, errorPredicate](auto fiber_input) {
            try {
                if (fiber_input.isValue()) {
                    auto input = fiber_input.underlying().template get<T>();
...
constexpr Task<T, E> Task<T, E>::onError(const std::function<void(const E&)>& handler) const noexcept {
    return Task<T, E>(op->flatMap([handler](auto fiber_input) {
        try {
            if (fiber_input.isValue()) {
                return fiber::FiberOp::value(fiber_input.underlying());
            } else if (fiber_input.isError()) {
...
constexpr Task<T, E> Task<T, E>::doOnCancel(const Task<None, None>& handler) const noexcept {
    return Task<T, E>(op->flatMap([handler_op = handler.op](auto fiber_input) {
        if (fiber_input.isValue()) {
            return fiber::FiberOp::value(fiber_input.underlying());
        } else if (fiber_input.isError()) {
            return fiber::FiberOp::error(fiber_input.underlying());
...
template <class T, class E>
template <class T2, typename std::enable_if<std::is_assignable<Either<T2, E>, T>::value>::type*>
constexpr Task<T2, E> Task<T, E>::dematerialize() const noexcept {
    return Task<T2, E>(op->flatMap([](auto fiber_input) constexpr {
        if (fiber_input.isValue()) {
...
constexpr Task<T, E> Task<T, E>::recover(const std::function<T(const E&)>& predicate) const noexcept {
    return Task<T, E>(op->flatMap([predicate](auto fiber_input) {
        if (fiber_input.isValue()) {
            return fiber::FiberOp::value(fiber_input.underlying());
        } else if (fiber_input.isError()) {
            auto input = fiber_input.underlying().template get<E>();
...
constexpr Task<T, E> Task<T, E>::sideEffect(const Task<T2, E>& task) const noexcept {
    return flatMap<T>([task](T result) constexpr {
        return task.template map<T>([result](T2) constexpr { return result; });
    });
}
...
constexpr Task<T, E> Task<T, E>::guarantee(const Task<T2, E>& task) const noexcept {
    return Task<T, E>(op->flatMap([guaranteed_op = task.op](auto fiber_value) constexpr {
