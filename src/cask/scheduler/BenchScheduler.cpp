...
CancelableRef BenchScheduler::submitAfter(int64_t milliseconds, const std::function<void()>& task) {
    ...
    auto cancelable = std::make_shared<BenchCancelableTimer>(this->shared_from_this(), id);
...
BenchScheduler::BenchCancelableTimer::BenchCancelableTimer(const std::shared_ptr<BenchScheduler>& parent, int64_t id)
    : parent(parent)
    , id(id)
    , callbacks()
    , callback_mutex()
